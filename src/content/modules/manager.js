...
var manager = {
  ...
  /**
   * Class used by the folderlist in the main account settings tab
   */
  FolderListView: class {
    ...
    // Checkif an image is an absolute or relative path. If relative, prepend TbSync.
    checkImage(url) {
      return url.includes("://")
      ? url
      : "chrome://tbsync/" + url;
     ...
     * @param folderListRowData         [in] TODO
     */        
    async getRow(document, folderListRowData) {
      //create checkBox for select state
      let accountData = new TbSync.AccountData(folderListRowData.accountID);
      let folderData = new TbSync.FolderData(accountData, folderListRowData.folderID);
      ...
      itemType.setAttribute("src", this.checkImage(folderListRowData.typeImage));
      itemType.setAttribute("style", "margin: 0px 9px 0px 3px;");

      //ACL
      let roAttributes = folderListRowData.attributesRoAcl;
      let rwAttributes = folderListRowData.attributesRwAcl;
     ...
     */        
    async updateRow(document, listItem, folderListRowData) {
      let foldername = folderListRowData.folderDisplayName;
      let accountData = new TbSync.AccountData(folderListRowData.accountID);
      let folderData = new TbSync.FolderData(accountData, folderListRowData.folderID);
