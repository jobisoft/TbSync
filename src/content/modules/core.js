...
var core = {
  ...
  getNextPendingFolder: async function (syncData) {
    let sortedFolders = await TbSync.providers.request(syncData.accountData.getAccountProperty("provider"), "Base.getSortedFolders", [TbSync.providers.getAccountObject(syncData.accountData)]);
    for (let i=0; i < sortedFolders.length; i++) {
      if (sortedFolders[i].getFolderProperty("status") != "pending") continue; //BROKEN
  ...
  syncAccount: async function (accountID, aSyncDescription = {}) {
    ...
    do {
      ...
      if (syncDescription.syncList) {
        ...
        if (listStatusData.version != TbSync.apiVersion) {
