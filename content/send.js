...
var EXPORTED_SYMBOLS = ["Send","callback"];
    tzprefs = Components.classes["@mozilla.org/preferences-service;1"]
...
function myDump(what,aMessage) {
		...
		Components.utils.import("chrome://tzpush/content/toxml.js");
		  ...
		  function decode_utf8(s) {
			var appInfo = Components.classes["@mozilla.org/xre/app-info;1"]
                        .getService(Components.interfaces.nsIXULAppInfo);
                  var platformVer = appInfo.platformVersion;
         if (platformVer >= 40) {return s}
         else { try {return decodeURIComponent(escape(s))}
         catch(e) {return s}
			 }
			 }
		
		if (this.prefs.getCharPref("debugwbxml") === "1") {this.myDump("sending",decodeURIComponent(escape(toxml(wbxml).split('><').join('>\n<'))))
		writewbxml(wbxml)
	...
	else {req.setRequestHeader("MS-ASProtocolVersion", "14.0")}
    ...
    { //this.myDump("header",req.getAllResponseHeaders().toLowerCase())
	 ...
	 if (this.prefs.getCharPref("debugwbxml") === "1") {this.myDump("recieved",decode_utf8(toxml(wbxml).split('><').join('>\n<')))
	 writewbxml(wbxml)
         //this.myDump("header",req.getAllResponseHeaders().toLowerCase())
         } 
		...
		else {this.myDump("tzpush request status","449 -- Insufficient information - retry with provisioning")}
	}
     ...
     if (SSL === true) {var hthost = "https://" + host}
        ...
        try {
            ...
            if (this.prefs.getCharPref("synckey") === '') {this.GetFolderId()}
            else {this.fromzpush()}
            }
		...
		else { this.myDump("tzpush request status","reported -- " + req.status)
        ...
        try {
            ...
            var platformVer = appInfo.platformVersion;
			if (platformVer >= 50) {
			var nBytes = wbxml.length, ui8Data = new Uint8Array(nBytes);
